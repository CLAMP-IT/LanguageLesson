  ...
  this.LanguageLesson.module("LessonAttemptsApp.Review", function(Review, App, Backbone, Marionette, $, _) {
    return Review.Controller = (function(_super) {
      ...
      Controller.prototype.initialize = function(options) {
        return RecorderControls.initialize();
      };

      Controller.prototype.review = function(lesson_attempt_id) {
        return App.request("lesson_attempt:entity", lesson_attempt_id, (function(_this) {
          return function(lesson_attempt) {
            console.log(lesson_attempt);
            _this.layout = _this.getLayoutView(lesson_attempt);
            _this.layout.on("show", function() {
              _this.showAttemptInfo(lesson_attempt);
              return _this.showQuestionAttemptList(lesson_attempt);
            });
      ...
      Controller.prototype.showQuestionAttemptList = function(lesson_attempt) {
        var questionAttemptsView;
        questionAttemptsView = this.getQuestionAttemptsView(lesson_attempt);
        return this.layout.questionAttemptsRegion.show(questionAttemptsView);
      };

      Controller.prototype.showAttemptInfo = function(lesson_attempt) {
        var infoView;
        infoView = this.getInfoView(lesson_attempt);
        return this.layout.infoRegion.show(infoView);
      };

      Controller.prototype.showQuestionAttemptLayout = function() {
        this.question_attempt_layout = new Review.QuestionAttemptLayout;
        return this.layout.questionAttemptLayoutRegion.show(this.question_attempt_layout);
      };

      Controller.prototype.getLayoutView = function(lesson_attempt) {
        return new Review.Layout;
      };

      Controller.prototype.getInfoView = function(lesson_attempt) {
        return new Review.LessonAttemptInfo({
          model: lesson_attempt
        });
      };

      Controller.prototype.getQuestionAttemptsView = function(lesson_attempt) {
        var questionAttemptsView;
        questionAttemptsView = new Review.QuestionAttempts({
          collection: lesson_attempt.get('question_attempts')
        });
        this.listenTo(questionAttemptsView, "childview:respond:question_attempt:clicked", function(iv, args) {
          var question_attempt;
          question_attempt = args.model;
          this.question_attempt_layout = new Review.QuestionAttemptLayout({
            model: question_attempt
          });
          this.layout.questionAttemptLayoutRegion.show(this.question_attempt_layout);
          return this.question_attempt_layout.showQuestionAttempt(args.model);
        });
        return questionAttemptsView;
      };
