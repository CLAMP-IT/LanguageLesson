...
(function() {
  ...
  this.LanguageLesson.module("LessonAttemptsApp.Review", function(Review, App, Backbone, Marionette, $, _) {
    return Review.QuestionAttemptLayout = (function(_super) {
      __extends(QuestionAttemptLayout, _super);

      function QuestionAttemptLayout() {
        return QuestionAttemptLayout.__super__.constructor.apply(this, arguments);
      }

      QuestionAttemptLayout.prototype.template = "lesson_attempts/review/templates/question_attempt_layout";

      QuestionAttemptLayout.prototype.regions = {
        questionAttemptRegion: "#question-attempt-region",
        responsesRegion: "#responses-region"
      };

      QuestionAttemptLayout.prototype.showQuestionAttempt = function(question_attempt) {
        this.question_attempt_view = new Review.QuestionAttempt({
          model: question_attempt
        });
        this.questionAttemptRegion.show(this.question_attempt_view);
        this.responses_view = new Review.QuestionAttemptResponses({
          collection: question_attempt.get('responses')
        });
        this.listenTo(this.responses_view, "childview:question_attempt_response:selected", function(iv, args) {
          return this.question_attempt_view.activateRegion(args.model);
        });
        return this.responsesRegion.show(this.responses_view);
      };

      return QuestionAttemptLayout;
