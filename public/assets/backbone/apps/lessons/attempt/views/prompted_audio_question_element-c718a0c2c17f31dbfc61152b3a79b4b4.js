...
(function() {
  ...
  this.LanguageLesson.module("LessonsApp.Attempt", function(Attempt, App, Backbone, Marionette, $, _) {
    return Attempt.PromptedAudioQuestionElement = (function(_super) {
      __extends(PromptedAudioQuestionElement, _super);

      function PromptedAudioQuestionElement() {
        return PromptedAudioQuestionElement.__super__.constructor.apply(this, arguments);
      }

      PromptedAudioQuestionElement.prototype.template = 'lessons/attempt/templates/_prompted_audio_question';

      PromptedAudioQuestionElement.prototype.events = {
        'click .js-record-begin': 'startRecording',
        'click .js-record-end': 'stopRecording'
      };

      PromptedAudioQuestionElement.prototype.onShow = function() {
        var wavesurfer;
        App.request("find:question_attempt:entity", this.options['attempt'].attributes['id'], this.model.attributes['element_id'], this.options['user'].attributes['id'], (function(_this) {
          return function(question_attempt) {
            _this.question_attempt = question_attempt;
            if (!question_attempt.attributes['id']) {
              ...
              console.log(_this.question_attempt.attributes['recordings'][0].url);
              return _this.showRecording(_this.question_attempt.attributes['recordings'][0].url);
            }
          };
        })(this));
        ...
        return wavesurfer.on('selection-update', function(selection) {
          return console.log(selection);
        });
      };

      PromptedAudioQuestionElement.prototype.onDestroy = function() {
        return console.log('closing');
      };

      PromptedAudioQuestionElement.prototype.startRecording = function() {
        RecorderControls.startRecording();
        return $('.lesson_element').addClass('recording');
      };

      PromptedAudioQuestionElement.prototype.stopRecording = function() {
        RecorderControls.stopRecording();
        RecorderControls.exportWAV((function(_this) {
          return function(blob) {
            var form, oReq, postUrl, url;
            url = URL.createObjectURL(blob);
            _this.showRecording(url);
            form = new FormData();
            form.append("recording[file]", blob, 'recording.wav');
            form.append("[question_attempt][lesson_attempt_id]", _this.options['attempt'].attributes['id']);
            form.append("[question_attempt][question_id]", _this.model.attributes['element_id']);
            form.append("[question_attempt][user_id]", _this.options['user'].attributes['id']);
            postUrl = Routes.add_lesson_attempt_question_attempts_path(_this.options['attempt'].attributes['id'], {
              format: 'json'
            });
      ...
      PromptedAudioQuestionElement.prototype.showRecording = function(url) {
        var au, hf, li;
        ...
        li.appendChild(au);
        return $('#recordings-list').append(li);
      };

      return PromptedAudioQuestionElement;
